...
│   ├── multistreamRelay.js   # MediaMTX HTTP API client — adds/removes per-streamer ffmpeg forwarder paths
│   ├── youtubeBroadcast.js   # YouTube Data API v3 broadcast lifecycle — getOrCreateLiveStream, createBroadcast, bind, transition (live/complete)
│   ├── streamKeyCrypto.js    # AES-256-GCM helpers for at-rest encryption of platform stream keys
│   ├── twitchRewards.js      # Twitch Channel Points Helix client — list/create/update/delete custom rewards, fulfill/cancel redemptions, syncRewards reconciliation, Affiliate-gate detection (403 → { affiliateGated: true })
│   ├── redemptionDispatcher.js # Channel point redemption flow — file-exists check, overlayBus emit, optional chat announcement, auto-fulfill timer, history log
│   └── redemptionTemplates.js  # Pure helper formatChatTemplate({user},{title},{cost}) with regex single-pass replacement (no double-replacement bugs)
├── routes/
...
│   ├── multistream.js    # Multistream dashboard page + MediaMTX webhooks (on-ready / on-not-ready)
...
└── views/
    ...
    ├── redemptions-config.ejs # Channel Point Rewards — affiliate/scope banners, master toggle, sync button, drag-reorder reward cards, add/edit modal with media upload + position picker + live preview, recent redemptions table
    ├── multistream.ejs       # Multistream dashboard — Twitch/YouTube OAuth status, ingest URL + stream key, YouTube broadcast defaults, OBS settings hint
...
└── redemptions/          # Per-streamer audio/video clips for channel point rewards (data/redemptions/{streamerId}/{rewardId}_{timestamp}.{ext})
...
- **Channel Point Rewards:** Native Twitch Channel Points → audio/video clips on OBS overlay. Streamer creates a reward in `/dashboard/redemptions` (title 2–45, cost 1–1,000,000, optional cooldown/prompt, audio mp3/wav/ogg ≤5MB or video mp4/webm ≤20MB, position preset or custom rect, volume, max-play 1–60s, chat announcement template `{user}/{title}/{cost}`, auto-fulfill toggle). Backend POSTs to `/helix/channel_points/custom_rewards` and stores the returned `reward.id` in `channel_point_rewards`. Files saved at `data/redemptions/{streamerId}/{rewardId}_{ts}.{ext}` and served via `/redemptions/*` static route (Cache-Control 24h; replacement uploads delete old file). Viewer redeems on Twitch → EventSub fires `channel.channel_points_custom_reward_redemption.add` → `eventsub.js` hands off to `redemptionDispatcher.handleRedemption()`: looks up reward by `twitch_reward_id`, verifies file on disk (auto-CANCELED + chat warning if missing), emits `redemption` event to `overlayBus`, optionally posts chat message, schedules auto-fulfill via Helix PATCH at `maxPlaySeconds + 1s`. Overlay's `playRedemption()` queues the event, plays the clip in dedicated `<audio id="redemption-audio">` / `<video id="redemption-video">` elements (separate from the alert-card path), with `finalVolume = overlayMaster × rewardVolume`. Test button (`POST /dashboard/redemptions/:id/test`) emits a synthetic redemption event straight to the overlay — no Twitch call, no chat send, logged with `is_test=1`. Requires `channel:manage:redemptions` scope on broadcaster auth — existing streamers see a yellow "Reconnect Twitch" banner until they re-auth. EventSub silently skips the redemption subscribe call for streamers without the scope (no log noise, other notifications keep working). Twitch Affiliate or Partner status required — `createReward` returns `{ affiliateGated: true }` on 403; UI shows red banner. `syncRewards()` reconciles local DB with upstream (Twitch is source of truth for title/cost/prompt; local-only fields are media file/position/volume/chat template). History log in `channel_point_redemptions` (outcome: played/refunded_no_file/disabled/error/test). DB columns added on `streamers`: `cp_rewards_enabled`, `cp_affiliate_status` (`unknown`/`ok`/`not_affiliate`), `cp_last_sync_at`. The dashboard route MUST pre-compute `hasScope` via `hasRedemptionScope(streamer)` from `services/twitch.js` and pass it as a template local — Express's EJS integration does NOT expose `require()` to templates, so calling `require('../services/twitch')` from within the EJS will throw `require is not defined` and crash the page.
- **Multistream (Twitch + YouTube passthrough):** Streamer points OBS at `rtmp://ingest.atletanotifications.com/live/<token>` (a self-hosted MediaMTX server, free on Oracle Cloud Always Free tier). MediaMTX fires `runOnReady` → `POST /api/multistream/webhook/on-ready` on Express, which: (1) fetches the streamer's Twitch RTMP key via Helix `streams/key` (requires `channel:read:stream_key` scope on broadcaster auth), (2) creates+binds a YouTube `liveBroadcast` via Data API v3 using stored defaults (privacy/category/optional default-title; falls back to current Twitch stream title), (3) calls MediaMTX `/v3/config/paths/add/*` to spawn one `ffmpeg -c copy` forwarder per destination — pure passthrough, no re-encoding, no PC-side cost. Async after the webhook returns: poll `liveStreams.status` until `streamStatus=active` then `transition→live`. On `runOnNotReady`: remove forwarders + `transition→complete` for the broadcast. Streamer dashboard at `/dashboard/multistream` shows OAuth connection status (Connect Twitch / Connect YouTube buttons — never paste-key UI), enable toggle, ingest URL + per-streamer stream key, YouTube broadcast defaults, recommended OBS settings (1080p60 @ 6 Mbps, NVENC). DB columns on `streamers`: `multistream_enabled`, `multistream_token` (unique base64url), `multistream_youtube_stream_id` (reusable YT liveStream id), `multistream_youtube_stream_key_enc` (AES-256-GCM via `services/streamKeyCrypto.js`, key from `MULTISTREAM_KEY_SECRET`), `multistream_youtube_privacy/category_id/default_title`, `multistream_active_broadcast_id`. Quota: ~200 YouTube units/session (insert + bind + 2× transition); default daily quota 10k = ~50 sessions/day before increase needed. See `docs/multistream-relay-setup.md` for the operator side (Oracle VM + MediaMTX install + Cloudflare Tunnel).
...
- `MEDIAMTX_API_URL`, `MEDIAMTX_INGEST_URL`, `MEDIAMTX_API_TOKEN`, `MEDIAMTX_WEBHOOK_SECRET` — multistream relay coordinates (see `docs/multistream-relay-setup.md`)
...
- Channel point reward media stored in `data/redemptions/{streamerId}/` (persistent volume), served via `/redemptions/` static route. Audio cap 5MB; video cap 20MB. Filenames are `{rewardId}_{timestamp}.{ext}` so cache headers (24h) are safe across replacements.
